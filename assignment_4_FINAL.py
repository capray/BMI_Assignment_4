...
# function to find columns with null values, so we can plot "___ is missing"
...
for sheet_num in range(11):
    ...
    # skip last Plate as there are no relevant columns
    if sheet_num != 10:
    ...
    for patient_name, patient_data in patient_id:
        ...
        if patient_data["Dilution"].isnull().any():
            continue
        # get list of null columns (not to plot)
        ...
        for plot_col in plot_columns:
            ...
            if "Standard" in patient_name:
                ...
                # for last sheet, no gender/age/hospital columns available
                if sheet_num == 10:
                    plt_title = plt_title + " " + plot_col
                else:
